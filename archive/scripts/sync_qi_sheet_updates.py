...
SHEET_NAME = "Excerpt Tool 1.20"
CLASPRC_PATH = Path.home() / ".clasprc.json"
DEFAULT_UPDATE_CSV = Path("data/qi_graphic_reconciliation_sheet_updates.csv")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Verify reconciliation-backed quote-image rows against the live Responses sheet and "
            "optionally mark them approved and created."
        )
    )
    parser.add_argument(
        "--update-csv",
        ...
        help=f"CSV of reconciliation-backed live sheet updates (default: {DEFAULT_UPDATE_CSV})",
    )
    parser.add_argument(
        "--db-path",
        type=Path,
    ...
    parser.add_argument(
        "--verify-only",
        action="store_true",
        help="Only verify the live rows and print a summary without writing updates",
    )
    parser.add_argument(
        "--limit",
        type=int,
...
def load_sheet_update_rows(path: Path, limit: int | None = None) -> list[dict[str, str]]:
...
def load_expected_source_rows(db_path: Path) -> dict[int, dict[str, str]]:
    connection = sqlite3.connect(db_path)
    connection.row_factory = sqlite3.Row
    try:
        rows = connection.execute(
            """
            SELECT source_row_number, author, book_title, poem_title, excerpt_text
            FROM source_rows
            """
        ).fetchall()
    finally:
    ...
    return {
        int(row["source_row_number"]): {
            ...
            "excerptText": row["excerpt_text"] or "",
        }
        for row in rows
    }
...
def fetch_live_rows(access_token: str, source_rows: list[int]) -> dict[int, dict[str, str]]:
    live_by_row: dict[int, dict[str, str]] = {}

    for index in range(0, len(source_rows), SHEETS_BATCHGET_CHUNK_SIZE):
        chunk = source_rows[index : index + SHEETS_BATCHGET_CHUNK_SIZE]
        ranges = [f"'{SHEET_NAME}'!Z{row}:BC{row}" for row in chunk]
        url = (
        ...
        for value_range in payload.get("valueRanges", []):
            ...
            padded = row + [""] * max(0, 29 - len(row))
            live_by_row[row_number] = {
                "author": padded[0],
                "poemTitle": padded[1],
                "excerptText": padded[2],
                "bookTitle": padded[3],
                "approved": padded[9],
                "statusIndicator": padded[10],
                "quoteCreatedQc": padded[11],
                "excerptReviewDecision": padded[28],
            }
    return live_by_row
...
def row_matches(expected: dict[str, str], live: dict[str, str]) -> tuple[bool, str]:
    checks = [
        ...
        (
            "excerptText",
            normalize_text(excerpt_prefix(expected["excerptText"])),
...
def build_update_payload(
    ...
    for row in verified_rows:
        source_row = row["approvedSourceRow"]
        data.append(
            {
                "range": f"'{SHEET_NAME}'!AI{source_row}:AK{source_row}",
                "majorDimension": "ROWS",
                "values": [["Y", "", "Y"]],
            }
        )
        data.append(
            {
                "range": f"'{SHEET_NAME}'!BC{source_row}",
                "majorDimension": "ROWS",
                "values": [["ACCEPT"]],
            }
        )
...
def main() -> int:
    args = parse_args()
    update_rows = load_sheet_update_rows(args.update_csv, limit=args.limit)
    expected_by_row = load_expected_source_rows(args.db_path)

    candidate_rows = []
    missing_expected_rows = []
    for row in update_rows:
        source_row = int(row["approvedSourceRow"])
        if source_row not in expected_by_row:
            missing_expected_rows.append(source_row)
            continue
        candidate_rows.append(row)

    access_token = refresh_access_token()
    live_by_row = fetch_live_rows(access_token, [int(row["approvedSourceRow"]) for row in candidate_rows])
    ...
    for row in candidate_rows:
        source_row = int(row["approvedSourceRow"])
        expected = expected_by_row[source_row]
        live = live_by_row.get(source_row)
        if not live:
            mismatches.append(
                {
                    "sourceRow": str(source_row),
                    "reason": "missing_live_row",
        ...
        matches, mismatch_field = row_matches(expected, live)
        if not matches:
            mismatches.append(
                {
                    "sourceRow": str(source_row),
                    "reason": f"mismatch_{mismatch_field}",
                    "expectedAuthor": expected["author"],
                    "liveAuthor": live["author"],
                }
            )
            continue

        if (
            clean_whitespace(live["approved"]).upper() == "Y"
            and clean_whitespace(live["quoteCreatedQc"]).upper() == "Y"
            and clean_whitespace(live["excerptReviewDecision"]).upper() == "ACCEPT"
        ):
            already_done += 1
            continue
    ...
    summary = {
        ...
        "missing_expected_rows": len(missing_expected_rows),
    ...
    if args.verify_only:
        print(
            json.dumps(
                {
                    "summary": summary,
                    "missingExpectedRows": missing_expected_rows[:25],
